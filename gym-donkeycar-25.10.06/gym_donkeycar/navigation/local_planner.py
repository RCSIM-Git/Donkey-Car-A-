...
class LocalPlanner:
    ...
    def reset_to_nearest(self, current_pose, path, start_search=True):
        """Znajduje najbliższy punkt na całej trasie. Jeśli start_search=True, ogranicza szukanie do początku ścieżki."""
        x, y, _ = current_pose
        best_dist = float('inf')
        nearest_idx = 0
        
        # Jeśli to start wyścigu, szukamy tylko w pierwszej połowie trasy, by nie "przeskoczyć" do końca
    ...
    def get_steering(self, current_pose, path, speed=0.0):
        """
        Oblicza skręt Pure Pursuit, bazując na indeksach (niezależnie od pętli).
        """
        # 1. Znajdź najbliższy punkt (Globalny reset tylko za pierwszym razem)
        ...
        # ROS2 Regulated Pure Pursuit: Dynamiczny wzrok
