...
class ICPOptimizer:
    """
    Optymalizator ICP wspomagany przez (opcjonalne) podpowiedzi ruchu.
    ICP Optimizer aided by (optional) motion hints.
    """

    def __init__(self, logger: logging.Logger):
        """
        Inicjalizuje optymalizator ICP.
    ...
    def align_with_imu_hint(
        ...
        """
        Dopasowuje chmurę źródłową do docelowej używając ICP.
        Align source cloud to target cloud using ICP.

        Args:
            source_cloud (np.ndarray): Tablica Nx3 punktów (bieżący skan).
            target_cloud (np.ndarray): Tablica Mx3 punktów (poprzedni skan / mapa).
            initial_transform (np.ndarray): Macierz 4x4 inicjalizująca.
