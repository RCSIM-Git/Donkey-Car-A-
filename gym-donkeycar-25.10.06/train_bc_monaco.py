...
# Obsługa wejścia 640x480 oraz rozszerzonego zestawu sensorów

class VRAMDataset:
    def __init__(self, data_path, device="cuda", train_size=(320, 240)):
        self.device = device
        self.img_w, self.img_h = train_size # Optymalizacja pod 16GB VRAM
        self.json_files = []
        
        print(f"Scanning tubs in {data_path}...")
        # Szukamy najnowszej tuby eksperckiej
        ...
        if not tubs:
            print("BŁĄD: Nie znaleziono tuby z danymi eksperckimi!")
            return

        target_tub = tubs[0]
        print(f"Ładowanie danych z: {target_tub}")
        ...
        print(f"Znaleziono {self.num_samples} rekordów.")
        
        if self.num_samples == 0: return

        # Alokacja pamięci (Uint8 dla obrazów, aby zaoszczędzić VRAM)
        # 30k * 640 * 480 * 3 = ~27 GB. Jeśli zabraknie VRAM, proces zostanie zabity.
        # W takim przypadku należy zmniejszyć num_samples lub przejść na standardowy DataLoader.
        print(f"Alokacja VRAM dla obrazów {self.img_w}x{self.img_h}...")
        self.images = torch.zeros((self.num_samples, self.img_h, self.img_w, 3), dtype=torch.uint8, device=device)
        self.lidars = torch.zeros((self.num_samples, 60), dtype=torch.float32, device=device) # Używamy 60 punktów dla lepszej precyzji
    ...
    def _load_item_to_gpu(self, i):
        try:
            ...
            # Lidar: Downsampling 180 -> 60 (co 3 stopnie przy inc=2.0)
            lidar_raw = data.get("lidar/raw", [0.0]*180) # Domyślnie 180 przy inc=2.0
...
class BCModel(nn.Module):
    def __init__(self):
        ...
        # CNN dla 320x240 (Zoptymalizowane pod Blackwell + Sync z PPO)
...
def train_monaco():
    ...
    batch_size = 64 # Zmniejszony batch size dla dużej rozdzielczości
    epochs = 100
