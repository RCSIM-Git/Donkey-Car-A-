...
class DonkeySimMsgHandler(IMesgHandler):
    ...
    def predict(self, image_array):
        # Normalize input as we removed the Lambda layer from the model
        normalized_img = image_array[None, :, :, :].astype(np.float32) / 255.0
        outputs = self.model.predict(normalized_img)
...
def go(filename, address, constant_throttle, gif):
    # No longer need safe_mode=False as Lambda layer is removed
