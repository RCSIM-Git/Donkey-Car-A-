...
class GridMapper:
    ...
    def __init__(
        ...
        self.L_OCC = 5.0   # Solid walls
        self.L_FREE = -0.5 # Optimal clearing (removes ghost rays while preserving walls)
        self.L_MAX = 8.0
        self.L_MIN = -8.0
    ...
    def clear(self) -> None:
        """Resets map to initial state (Unknown)."""
        self.grid.fill(127)
    ...
    def update(
        ...
        # 2. Convert to grid pixels (use floor for correct negative indices)
    ...
    def _update_grid_from_log_odds(self):
        ...
        # Wall dilation (1px thickening) to remove gaps in SLAM
        # Critical at 5cm resolution
        kernel = np.ones((3,3), np.uint8)
        # 0 is wall, so dilate black pixels
    ...
    def _resize_grid(self, padding_meters: float = 20.0):
        """Resizes grid to fit new coordinates while maintaining world consistency."""
        ...
        # Copy old map to middle of new map
