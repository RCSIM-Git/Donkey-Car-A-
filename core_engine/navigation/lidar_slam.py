...
class LidarSLAM:
    """
    Lidar SLAM system using Scan Matching (ICP) and Graph Optimization.
    """
    ...
    def process_scan(
        ...
        if prediction_pose:
            # Use GPS/IMU data as hint for ICP
    ...
    def _scan_to_cloud(self, scan_data: list) -> np.ndarray:
        """
        Converts scan data to point cloud Nx3 (z=0).
        """
        ...
        # Calculate projection (X=Right, Y=Forward)
        # Offset 0.0m according to monaco_mapper
        # Use negative sine to correct mirror reflection of DonkeyGym (CW)
