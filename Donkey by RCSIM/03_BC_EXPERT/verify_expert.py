...
ppo_dir = os.path.abspath(os.path.join(curr_dir, "..", "04_RL_PPO"))
sys.path.append(ppo_dir)
sys.path.append(r"d:\DonkeyPROJECT\MonacoRacing")
...
def verify():
    ...
    model = PPO("MultiInputPolicy", env, policy_kwargs=policy_kwargs, verbose=1, device=device)
    
    script_dir = os.path.dirname(os.path.abspath(__file__))
    bc_path = os.path.join(script_dir, "bc_model_weights_monaco.pth")
    if not os.path.exists(bc_path):
        bc_path = "bc_model_weights_monaco.pth"
        
    if os.path.exists(bc_path):
        print(f"Injecting weights from {bc_path}...")
        bc_state = torch.load(bc_path, map_location=device)
