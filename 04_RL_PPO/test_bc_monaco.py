...
# Resolution: 640x480 (Input) -> 320x240 (CNN)

class BCModel(nn.Module):
    def __init__(self):
        super(BCModel, self).__init__()
        # CNN must be identical to the one used in train_bc_monaco.py
...
def run_inference():
    ...
    conf = {
        ...
        "body_style": "f1", "car_name": "BLACKWELL_BC", "body_rgb": (0, 255, 0), # Green for AI
    ...
    print("Environment initialization (reset)...", flush=True)
    obs_dict, info = env.reset()
    
    print("--- STARTING ONLINE BC TEST (Control Loop) ---", flush=True)
    ...
    try:
        while True:
            ...
            # 1. Data Preparation (Vision/Sensors)
            t0 = time.time()
